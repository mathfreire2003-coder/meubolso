...
const CATEGORIES = ["Alimentação","Transporte","Saúde","Educação","Lazer","Vestuário","Casa","Tecnologia","Assinaturas","Carro","Outros"];
const CAT_COLORS = ["#4080D0","#22C4A0","#F5734A","#A855F7","#F59E0B","#EC4899","#10B981","#6366F1","#14B8A6","#DC2626","#94A3B8"];
...
async function load(key, fallback) { return dbLoad(key, fallback); } async function _load_unused(key, fallback) {
...
export default function App() {
  ...
  const totalInst = installments.reduce((s, i) => s + (i.monthlyAmount || 0), 0);
    ...
    <div className="fe" style={{ display:"flex", minHeight: 600, background: "#E8EEF8", borderRadius: 16, overflow:"hidden", border:"1px solid #D1D9E6" }}>
      ...
      <div style={{ flex: 1, display:"flex", flexDirection:"column", minWidth: 0, background: BG }}>
        ...
        <div style={{ flex: 1, padding:"22px 24px", overflowY:"auto" }}>
          ...
          {tab === "projecao" && <ProjecaoAnualTab installments={installments} fixedExpenses={fixedExpenses} goals={goals} settings={settings} transactions={transactions} />}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="metric-card">
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom: 10 }}>
        ...
        <div style={{ width: 28, height: 28, borderRadius: 8, background: "#F0F4FA", display:"flex", alignItems:"center", justifyContent:"center", color:"#64748B" }}>{icon}</div>
      </div>
...
function Dashboard({ installments, fixedExpenses, goals, settings, totalFixed, totalInst, totalGoalsMonth, balance, transactions }) {
  ...
  const monthTx = transactions.filter(t => t.date && t.date.startsWith(currentMonthKey));
  ...
  installments.forEach(i => { const c = i.category || "Outros"; catMap[c] = (catMap[c] || 0) + i.monthlyAmount; });
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap: 18 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4, 1fr)", gap: 14 }}>
        <MetricCard label="Renda Mensal" value={fmt(income)} sub="base de cálculo"
          valueColor="#1E293B"
        ...
        <MetricCard label="Despesas Fixas" value={fmt(totalFixed)} sub={`${fixedExpenses.filter(e=>e.active).length} ativas`}
          valueColor="#DC2626"
        ...
        <MetricCard label="Parcelamentos" value={fmt(totalInst)} sub={`${installments.length} compras`}
          valueColor="#D97706"
        ...
        <MetricCard label="Variáveis (mês)" value={fmt(totalVariable)} sub={`${monthTx.length} lançamento${monthTx.length !== 1 ? "s" : ""}`}
          valueColor="#7C3AED"
        ...
        <MetricCard label="Saldo Livre" value={fmt(balance - totalVariable)} sub={balance >= 0 ? "após variáveis" : "no vermelho"}
          valueColor={(balance - totalVariable) >= 0 ? "#16A34A" : "#DC2626"}
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap: 14 }}>
        ...
        <div className="card" style={{ padding:"16px 18px" }}>
          ...
          <p style={{ fontSize: 11, color:"#94A3B8", marginBottom: 14 }}>distribuição mensal</p>
          {pieData.length === 0
            ? <div className="empty"><p style={{ fontSize: 13, color:"#94A3B8" }}>Sem dados cadastrados</p></div>
            : <>
        ...
        {/* Cards breakdown */}
        <div className="card" style={{ padding:"16px 18px" }}>
          <p style={{ fontWeight: 600, fontSize: 13, color:"#1E293B", marginBottom: 4 }}>Gastos por cartão</p>
          <p style={{ fontSize: 11, color:"#94A3B8", marginBottom: 14 }}>parcelamentos ativos</p>
          {cardData.length === 0
            ? <div className="empty" style={{ padding:"28px 12px" }}><p style={{ fontSize: 13, color:"#94A3B8" }}>Nenhum cartão cadastrado</p></div>
            : cardData.map((d, i) => {
              const m = CARD_META[d.name] || { color:"#888", light:"#eee", text:"#555" };
              const maxV = cardData[0].value;
              return (
                <div key={i} style={{ marginBottom: 12 }}>
                  ...
                  <div className="progress-track">
                    <div style={{ height:"100%", width:`${(d.value/maxV)*100}%`, background: m.color, borderRadius: 3 }} />
                  </div>
                </div>
              );
            })
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap: 14 }}>
                ...
                <div key={i.id} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"9px 0", borderBottom:"1px solid #F1F5F9" }}>
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <div style={{ display:"flex", gap: 6, marginTop: 3, flexWrap:"wrap", alignItems:"center" }}>
                      ...
                      <span style={{ fontSize: 11, color:"#94A3B8" }}>{remaining}x restantes • {fmt(i.monthlyAmount)}/mês</span>
                    </div>
                  </div>
        ...
        <div className="card" style={{ padding:"16px 18px" }}>
          ...
          {goals.length === 0
            ? <p style={{ fontSize: 13, color:"#94A3B8" }}>Nenhuma meta cadastrada</p>
    ...
    <div style={{ marginBottom: 14 }}>
      ...
      <div style={{ display:"flex", flexWrap:"wrap", gap: 7 }}>
        ...
        <div onClick={() => onChange("")}
          style={{ cursor:"pointer", padding:"5px 12px", borderRadius: 20, fontSize: 11, fontWeight: 600, background:!value?"#EBF3FF":"#F8FAFC", color:!value?ACCENT:"#94A3B8", border:`1px solid ${!value?ACCENT:"#E2E8F4"}`, transition:"all 0.15s" }}>
          Sem cartão
        </div>
      </div>
    </div>
...
function PageHeader({ count, countLabel, total, totalLabel, totalColor, onAdd, addLabel }) {
  return (
    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom: 18 }}>
      <div style={{ display:"flex", gap: 20 }}>
        <div className="metric-card" style={{ padding:"10px 16px" }}>
          <p className="section-label" style={{ marginBottom: 2 }}>{countLabel}</p>
          <p style={{ fontSize: 18, fontWeight: 700, color:"#1E293B" }}>{count}</p>
        </div>
        <div className="metric-card" style={{ padding:"10px 16px" }}>
          <p className="section-label" style={{ marginBottom: 2 }}>{totalLabel}</p>
          <p style={{ fontSize: 18, fontWeight: 700, color: totalColor }}>{fmt(total)}</p>
        </div>
      </div>
...
function InstallmentsTab({ items, onChange }) {
  ...
  const blank = () => ({ id: Date.now(), name:"", totalAmount:"", paidInstallments:0, totalInstallments:"", monthlyAmount:"", startDate:"", category:"Outros", creditCard:"" });
  ...
  return (
    <div>
      <PageHeader count={items.length} countLabel="Parcelamentos" total={totalMonthly} totalLabel="Total/mês" totalColor="#D97706" onAdd={openAdd} addLabel="+ Adicionar" />

      {items.length === 0
        ? <div className="empty"><p style={{ fontWeight: 600, color:"#475569" }}>Nenhum parcelamento ainda</p><p style={{ fontSize: 12, color:"#94A3B8", marginTop: 4 }}>Adicione suas compras parceladas para acompanhar.</p></div>
        : (
          <div style={{ display:"flex", flexDirection:"column", gap: 10 }}>
            {items.map(item => {
              ...
              return (
                <div key={item.id} className="inst-row" style={{ borderLeft:`3px solid ${barColor}` }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom: 10 }}>
                    <div style={{ flex:1, minWidth:0 }}>
                      <p style={{ fontSize: 14, fontWeight: 700, color:"#1E293B", marginBottom: 5 }}>{item.name}</p>
                    ...
                    <div style={{ textAlign:"right", flexShrink:0, marginLeft: 14 }}>
                      <p style={{ fontSize: 16, fontWeight: 700, color:"#D97706" }}>{fmt(item.monthlyAmount)}<span style={{ fontSize: 11, fontWeight: 400, color:"#94A3B8" }}>/mês</span></p>
                  ...
                  <div style={{ marginBottom: 10 }}>
                    ...
                    <div className="progress-track">
                      <div style={{ height:"100%", width:`${pct}%`, background: pct >= 100 ? "#16A34A" : barColor, borderRadius: 3, transition:"width 0.4s" }} />
                    </div>
                  </div>
...
function FixedTab({ items, onChange }) {
  ...
  const openAdd = () => { setEditItem({ id: Date.now(), name:"", amount:"", category:"Outros", dueDay:"", active:true }); setModal("form"); };
  const openEdit = (item) => { setEditItem({ ...item }); setModal("form"); };
  const save = () => {
    if (!editItem.name) return;
    const exists = items.find(i => i.id === editItem.id);
  ...
  return (
    <div>
      <PageHeader count={items.filter(e=>e.active).length} countLabel="Ativas" total={total} totalLabel="Total/mês" totalColor="#DC2626" onAdd={openAdd} addLabel="+ Adicionar" />
...
function GoalsTab({ items, onChange }) {
  ...
  return (
    <div>
      <PageHeader count={items.length} countLabel="Metas" total={totalGoalsSaved} totalLabel="Total guardado" totalColor={ACCENT} onAdd={openAdd} addLabel="+ Nova meta" />
                ...
                <div key={item.id} className="goal-card" style={{ borderTop:`3px solid ${c}` }}>
                  ...
                  {depositId === item.id && (
                    <div style={{ display:"flex", gap: 8, marginBottom: 10 }}>
                      <input type="number" value={depositVal} onChange={e => setDepositVal(e.target.value)} placeholder="Valor (R$)" autoFocus onKeyDown={e => e.key === "Enter" && deposit(item.id)} style={{ flex:1 }} />
...
function buildProjection(installments, fixedExpenses, goals, settings) {
  ...
  return Array.from({ length: 12 }, (_, m) => {
    ...
    const activeInst = installments.filter(i => m < ((i.totalInstallments || 0) - (i.paidInstallments || 0)));
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap: 18 }}>
      ...
      <div className="card" style={{ padding:"18px 20px" }}>
        ...
        <p style={{ fontSize: 11, color:"#94A3B8", marginBottom: 16 }}>As colunas de despesas encolhem conforme os parcelamentos se encerram</p>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"12px 32px" }}>
                    ...
                    <div>
                      ...
                      {m.activeInst.length === 0
                        ? <p style={{ fontSize:12, color:"#94A3B8" }}>Nenhum parcelamento neste mês</p>
                        : m.activeInst.map(i => {
...
const PAYMENT_METHODS = ["Dinheiro","Débito","Santander","Nubank","Nubank PJ","Will Bank","Mercado Pago","Pan","Caixa","Pix","Outro"];
const PM_COLOR = { "Dinheiro":"#16A34A","Débito":"#0369A1","Pix":"#22C4A0","Outro":"#94A3B8",
  "Santander":"#CC2929","Nubank":"#8B5CF6","Nubank PJ":"#6D28D9","Will Bank":"#D97706","Mercado Pago":"#0EA5E9","Pan":"#1D4ED8","Caixa":"#0369A1" };
...
function LancamentosTab({ transactions, onChange, budgets, onBudgetsChange }) {
  ...
  // All months that have transactions + current month
  const allMonths = [...new Set([monthKey(now), ...transactions.map(t => t.date?.slice(0,7)).filter(Boolean)])].sort().reverse();

  const monthTx = transactions.filter(t => t.date?.startsWith(selMonth)).sort((a,b) => b.date.localeCompare(a.date));
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:18 }}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          ...
          <button className="btn-ghost" onClick={() => navMonth(1)} style={{ padding:"6px 12px", fontWeight:700 }}
            disabled={selMonth >= monthKey(now)}>›</button>
        </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3, 1fr)", gap:14 }}>
        ...
        <div className="metric-card">
          ...
          {catList.length > 0 ? <>
            <p style={{ fontSize:18, fontWeight:700, color:"#1E293B", letterSpacing:"-0.02em" }}>{catList[0][0]}</p>
            <p style={{ fontSize:11, color:"#94A3B8", marginTop:3 }}>{fmt(catList[0][1])} · {totalMonth > 0 ? ((catList[0][1]/totalMonth)*100).toFixed(0) : 0}% do total</p>
          </> : <p style={{ fontSize:13, color:"#94A3B8" }}>—</p>}
        </div>
        {/* Top payment */}
        <div className="metric-card">
          <p className="section-label" style={{ marginBottom:8 }}>Meio mais usado</p>
          {Object.keys(byPM).length > 0 ? (() => {
            const top = Object.entries(byPM).sort((a,b)=>b[1]-a[1])[0];
            return <>
              <p style={{ fontSize:18, fontWeight:700, color: PM_COLOR[top[0]]||"#1E293B", letterSpacing:"-0.02em" }}>{top[0]}</p>
              <p style={{ fontSize:11, color:"#94A3B8", marginTop:3 }}>{fmt(top[1])}</p>
            </>;
          })() : <p style={{ fontSize:13, color:"#94A3B8" }}>—</p>}
        </div>
      </div>
                ...
                <div key={cat} style={{ marginBottom:10 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                    ...
                    <div style={{ textAlign:"right" }}>
                      ...
                      {budget && <span style={{ fontSize:10, color: over?"#DC2626":"#94A3B8", marginLeft:4 }}>/ {fmt(budget)}</span>}
                    </div>
                  </div>
                  <div className="progress-track">
                    <div style={{ height:"100%", width:`${Math.min(100,pct)}%`, background: over?"#DC2626":catColor, borderRadius:3 }} />
                  </div>
                  {over && <p style={{ fontSize:10, color:"#DC2626", marginTop:2, fontWeight:600 }}>Estourou {fmt(val-budget)} acima do orçamento</p>}
                </div>
              );
            })}
          </div>
...
function BudgetPanel({ budgets, onSave, catList, totalMonth }) {
  ...
  const allCats = [...new Set([...CATEGORIES, ...catList.map(c=>c[0])])];
  return (
    <div className="card" style={{ padding:"16px 18px" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
        <div>
          <p style={{ fontWeight:700, fontSize:13, color:"#1E293B" }}>Orçamento por categoria</p>
          <p style={{ fontSize:11, color:"#94A3B8", marginTop:2 }}>Defina limites mensais para acompanhar seus gastos por categoria</p>
        </div>
        <button className="btn-primary" onClick={() => onSave(local)} style={{ padding:"7px 16px", fontSize:12 }}>Salvar orçamentos</button>
      </div>
        ...
        {CATEGORIES.map(cat => {
          ...
          return (
            <div key={cat} style={{ background:"#F8FAFC", borderRadius:8, padding:"10px 12px", border: over?"1px solid #FECACA":"1px solid #F1F5F9" }}>
              ...
              <input type="number" value={budget} onChange={e => setLocal({ ...local, [cat]: e.target.value })}
                placeholder="Sem limite" style={{ fontSize:12, padding:"5px 8px", marginBottom:4 }} />
